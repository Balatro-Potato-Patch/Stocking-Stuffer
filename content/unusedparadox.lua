		...
		if context.before and StockingStuffer.first_calculation and not context.blueprint then
			...
			for _, v in ipairs(context.full_hand) do
				if v.ability.set == "Default" then -- intentionally disrespecting quantum enhancements
					cards_enhanced = true
					v:set_ability(
						SMODS.poll_enhancement({ key = "UnusedParadox_stocking_sugar_cookies", guaranteed = true })
					)
			...
			if cards_enhanced then
				return { message = localize("unusedparadox_frosted") }
			end
		end
	end,
	pos = { x = 2, y = 0 },
...
StockingStuffer.Present({
	...
	calculate = function(self, card, context)
		...
		elseif
			...
			and StockingStuffer.second_calculation
...
StockingStuffer.Present({
	...
	loc_vars = function(self, info_queue, card)
		return {
			vars = { card.ability.extra.mult, card.ability.extra.mult_loss },
			key = self.key .. (card.ability.extra.active and "_active" or "_inactive"),
		}
	end,
	calculate = function(self, card, context)
...
StockingStuffer.Present({
	...
	blueprint_compat = true,
	config = { extra = { items_left = 20, update_check = false } },
	loc_vars = function(self, info_queue, card)
		return { vars = { card.ability.extra.items_left } }
	end,
	update = function(self, card, dt)
		if card.ability.extra.update_check and card.area == G.stocking_present and G.shop_booster and G.shop_booster.cards and G.shop_jokers and G.shop_jokers.cards and G.STAGE == G.STAGES.RUN and G.STATES.SMODS_BOOSTER_OPENED ~= G.STATE and card.ability.extra.items_left > 0 then
		    local couponed = false
		    for _, area in ipairs({G.shop_booster.cards, G.shop_jokers.cards}) do
		        for _, v in ipairs(area) do
		            if not v.ability.couponed and not v.ability.stocking_gingerbreaded then
		                v.ability.stocking_gingerbreaded = true
                        couponed = true
		                G.E_MANAGER:add_event(Event({
		                    func = function()
		                        v.ability.couponed = true
		                        v.ability.stocking_gingerbreaded = false
		                        v:set_cost()
		                        return true
		                    end
		                }))
	...
	calculate = function(self, card, context)
		if (context.starting_shop or context.reroll_shop) and StockingStuffer.second_calculation then
			G.E_MANAGER:add_event(Event({
				func = function()
					local couponed = false
					for _, area in ipairs({ G.shop_booster.cards, G.shop_jokers.cards }) do
					...
					if couponed then
						SMODS.calculate_effect({ message = localize("unusedparadox_amazingdeals") }, card)
					end
					G.E_MANAGER:add_event(Event({
						trigger = "after",
						delay = 0.2,
						blocking = false,
						func = function()
							card.ability.extra.update_check = true
							return true
						end,
					}))
                    return true
		...
		if context.ending_shop then
			card.ability.extra.update_check = false
		end
	end,
	pos = { x = 5, y = 0 },
